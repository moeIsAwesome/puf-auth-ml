...
model_filename = './reports/trained_with_intact_full_length/svm_model_trained_with_intact.pkl'
...
# Assign "random/unknown/None" class if confidence < 90%
y_test_pred_with_conf = [
    3 if confidence < 0.9 else pred 
    for confidence, pred in zip(test_confidences, y_test_pred)
]

# Update label mapping to include "random/unknown/None"
label_mapping = {0: 'RPi1', 1: 'RPi2', 2: 'RPi3', 3: 'random/unknown/None'}
mapped_labels = [label_mapping[label] for label in test_labels_all]
mapped_predictions = [label_mapping[pred] for pred in y_test_pred_with_conf]

# Recalculate metrics with the new class included
test_accuracy = accuracy_score(mapped_labels, mapped_predictions)
test_precision = precision_score(mapped_labels, mapped_predictions, average='weighted')
test_recall = recall_score(mapped_labels, mapped_predictions, average='weighted')
test_f1 = f1_score(mapped_labels, mapped_predictions, average='weighted')
...
# Plot test metrics
plot_metrics(mapped_labels, mapped_predictions, "Test", test_confidences)
...
plot_confusion_matrix(mapped_labels, mapped_predictions, "Test")
